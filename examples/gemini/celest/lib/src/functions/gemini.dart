...
const _availableModels = ['gemini-2.0-flash'];
...
extension on GenerateContentResponse {
  Map<String, Object?> toJson() => {
    'promptFeedback': {
      ...
      'safetyRatings': [
        for (final rating in promptFeedback?.safetyRatings ?? <SafetyRating>[])
          {
      ...
      for (final candidate in candidates)
        {
          ...
          'safetyRatings': [
            for (final rating in candidate.safetyRatings ?? <SafetyRating>[])
              {
                'category': rating.category.name,
                'probability': rating.probability.name,
              },
