...
import 'package:crypto_keys/crypto_keys.dart'
    show AlgorithmIdentifier, PublicKey, Signature, SymmetricKey, Verifier;
...
final class SupabaseTokenVerifier {
  ...
  /// at the given [url].
  SupabaseTokenVerifier({required String url, Uint8List? jwtSecret})
    : _userUri = Uri.parse(url).resolve('./auth/v1/user'),
      _jwtSecret = switch (jwtSecret) {
        final Uint8List secret? => SymmetricKey(keyValue: secret),
        _ => null,
      };
  ...
  User? _verifyLocally(String token) {
    final SymmetricKey? jwtSecret = _jwtSecret;
    if (jwtSecret == null) {
      return null;
    }
    ...
    final AlgorithmIdentifier? alg = switch (_decodeJwtPart(header)) {
      {'alg': final String alg} => AlgorithmIdentifier.getByJwaName(alg),
    ...
    final Verifier<PublicKey> verifier = jwtSecret.createVerifier(alg);
