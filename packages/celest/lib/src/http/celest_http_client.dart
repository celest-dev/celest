import 'dart:io';

import 'package:async/async.dart';
import 'package:celest_core/src/util/globals.dart';
import 'package:http/http.dart' as http;
import 'package:multicast_dns/multicast_dns.dart';

final class CelestHttpClient extends http.BaseClient {
  CelestHttpClient({
    required this.projectName,
    http.Client? baseClient,
  }) : _baseClient = baseClient ?? http.Client();

  final String projectName;
  final http.Client _baseClient;
  final MDnsClient _mdnsClient = MDnsClient();
  late final Future<void> _mdnsClientStarted = _mdnsClient.start(
    listenAddress: InternetAddress.loopbackIPv4,
  );
  final AsyncCache<(String host, int port)> _celestIp = AsyncCache.ephemeral();

  @override
  Future<http.StreamedResponse> send(http.BaseRequest request) async {
    print('Sending request to ${request.url}');
    if (kIsWeb || !(Platform.isAndroid || Platform.isIOS)) {
      print('Sending request to localhost');
      return _baseClient.send(request);
    }
    print('Sending request to celest.local');
    assert(!request.finalized);
    final (host, port) = await _celestIp.fetch(() async {
      await _mdnsClientStarted;
      print('Resolving celest.local service');
      final ptr = await _mdnsClient
          .lookup<PtrResourceRecord>(
            ResourceRecordQuery.serverPointer('_celest._tcp'),
          )
          .firstWhere(
            (ptr) => ptr.domainName.startsWith('$projectName.'),
          );
      print('Found PTR: $ptr');
      final srv = await _mdnsClient
          .lookup<SrvResourceRecord>(
            ResourceRecordQuery.service(ptr.domainName),
          )
          .first;
      print('Found SRV: $srv');
      return (srv.target, srv.port);
    });
    final url = Uri(
      scheme: 'http',
      host: host,
      port: port,
      path: request.url.path,
      query: request.url.query,
      fragment: request.url.fragment,
    );
    final newRequest = switch (request) {
      final http.Request request => http.Request(request.method, url)
        ..headers.addAll(request.headers)
        ..bodyBytes = request.bodyBytes,
      _ => http.StreamedRequest(request.method, url)
        ..headers.addAll(request.headers)
        ..sink.addStream(request.finalize()),
    };
    return _baseClient.send(newRequest);
  }
}
