...
void sharedTests(String name, PlatformStorageFactory factory) {
  group(name, () {
    const disallowedScopes = ['split/scope'];
    for (final scope in disallowedScopes) {
      test('throws when namespace contains a disallowed pattern', () {
        expect(
          () => factory(namespace: scope),
          throwsArgumentError,
        );
      });

      test('throws when scope contains a disallowed pattern', () {
        expect(
          () => factory(namespace: 'test', scope: scope),
          throwsArgumentError,
        );
      });
    }

    const allowedScopes = [null, 'scope', 'com.domain.scope'];
    for (final scope in allowedScopes) {
      late String key;
      final storage = factory(namespace: 'test', scope: scope);
