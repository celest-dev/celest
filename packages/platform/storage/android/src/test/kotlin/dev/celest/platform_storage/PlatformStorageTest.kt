...
class DefaultsTest : BaseSecureStorageTest()

class CustomNamespaceTest : BaseSecureStorageTest() {
    override val namespace = "my.custom.namespace"
}

class CustomScopeTest : BaseSecureStorageTest() {
    override val namespace = "my.custom.namespace"
    override val scope = "custom-scope"
}

class EmptyScopeTest : BaseSecureStorageTest() {
    override val namespace = "my.custom.namespace"
    override val scope = ""

    @Test
    fun sharesWithNullScope() {
        val localNull = PlatformLocalStorage(context, namespace, null)
        localStorage.write("hello", "world")
        assertEquals("world", localNull.read("hello"))

        // TODO: Not working
//        val secureNull = PlatformSecureStorage(context, namespace, "")
...
//        assertEquals("world", secureNull.read("hello"))
    ...
    @Test
    fun twoInstances() {
        val inst1 = PlatformSecureStorage(context, defaultNamespace, "")
        val inst2 = PlatformSecureStorage(context, defaultNamespace, "")
...
abstract class BaseSecureStorageTest {
    ...
    private lateinit var secureStorage: PlatformSecureStorage
    lateinit var localStorage: PlatformLocalStorage
    private lateinit var storages: List<PlatformStorage>

    @Before
    fun setUp() {
        println("${javaClass.simpleName}: namespace=$namespace, scope=$scope")
        FakeAndroidKeyStore.setup
        secureStorage = PlatformSecureStorage(context, namespace, scope)
        localStorage = PlatformLocalStorage(context, namespace, scope)
