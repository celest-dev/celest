...
void main() {
  ...
  group('EnvManager', () {
    test('overlay', () async {
      final projectDir =
          fileSystem.systemTempDirectory.createTempSync('project');
...
''');
      ...
      await check(localManager.valueFor('PROJECT_ID'))
          .completes((it) => it.equals('local'));
      await check(localManager.valueFor('PROJECT_NAME'))
          .completes((it) => it.equals('Test Project'));

      await check(prodManager.valueFor('PROJECT_ID'))
          .completes((it) => it.equals('test'));
      await check(prodManager.valueFor('PROJECT_NAME'))
          .completes((it) => it.equals('Test Project'));
    });
