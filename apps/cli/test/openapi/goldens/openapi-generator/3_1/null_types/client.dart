// ignore_for_file: type=lint, unused_local_variable, unnecessary_cast, unnecessary_import

// ignore_for_file: no_leading_underscores_for_library_prefixes
import 'dart:convert' as _i2;

import 'package:celest/celest.dart' as _i3;
import 'package:http/http.dart' as _i1;

extension type ExampleServers(Uri _) implements Uri {
  static final ExampleServers defaultServer = server0;

  static final ExampleServers server0 =
      ExampleServers(Uri.parse(r'http://api.example.xyz/v1'));
}

/// Response type for [ExampleClient.list].
///
/// This is a marker interface implemented by all response types:
/// - [Person]
sealed class ListResponse {}

final class ExampleClient {
  ExampleClient({
    _i1.Client? httpClient,
    Uri? baseUri,
  })  : _httpClient = httpClient ?? _i1.Client(),
        _baseUri = baseUri ?? ExampleServers.defaultServer;

  final _i1.Client _httpClient;

  /// The base URI for the API
  final Uri _baseUri;

  Future<T> _decodeJson<T>(_i1.StreamedResponse $response) async {
    final $body = await $response.stream.bytesToString();
    final $json = _i2.jsonDecode($body);
    return _i3.Serializers.instance.deserialize<T>($json);
  }

  Future<ListResponse> list({required String personId}) async {
    final $uri = Uri.parse('$_baseUri/person/display/$personId');
    final $request = _i1.Request(
      'GET',
      $uri,
    );
    $request.headers['accept'] = 'application/json';
    final $response = await _httpClient.send($request);
    final $contentType = $response.headers['content-type'];
    switch ($response.statusCode) {
      /// OK
      case 200:
        return _decodeJson<Person?>($response);
      default:
        throw Exception('Unexpected response');
    }
  }
}

final class String$ implements Person {
  const String$(this.value);

  final String value;
}

final class Person implements ListResponse {
  Person({
    this.type,
    this.lastName,
    this.firstName,
    this.maidenName,
    this.address,
    this.secondHomeAddress,
    this.skills,
    this.nullableSkillsVariant2,
  });

  final String? type;

  final String? lastName;

  final String? firstName;

  final Person? maidenName;

  final Person? address;

  final NullableAddress? secondHomeAddress;

  final Person? skills;

  final Person? nullableSkillsVariant2;
}

final class Adult extends Person {
  Adult({
    this.lastName,
    this.firstName,
    this.maidenName,
    this.address,
    this.secondHomeAddress,
    this.skills,
    this.nullableSkillsVariant2,
    this.children,
    this.nullableChildrenVariant2,
  }) : super(type: 'Adult');

  final String? lastName;

  final String? firstName;

  final Person? maidenName;

  final Person? address;

  final NullableAddress? secondHomeAddress;

  final Person? skills;

  final Person? nullableSkillsVariant2;

  final Adult? children;

  final Adult? nullableChildrenVariant2;
}

final class Child extends Child {
  Child({
    this.age,
    this.lastName,
    this.firstName,
    this.maidenName,
    this.address,
    this.secondHomeAddress,
    this.skills,
    this.nullableSkillsVariant2,
  }) : super(type: 'Child');

  final int? age;

  final String? lastName;

  final String? firstName;

  final Person? maidenName;

  final Person? address;

  final NullableAddress? secondHomeAddress;

  final Person? skills;

  final Person? nullableSkillsVariant2;
}

final class NullableAddress {
  NullableAddress({
    this.street,
    this.city,
  });

  final String? street;

  final String? city;
}
