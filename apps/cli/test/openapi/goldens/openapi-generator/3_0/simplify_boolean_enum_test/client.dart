...
final class ExampleClient {
  ...
  Future<ListResponse> list({required String personId}) async {
    ...
    switch ($response.statusCode) {
      /// OK
      case 200:
        return _decodeJson<BooleanEnumTest?>($response);
      default:
...
final class BooleanEnumTest implements ListResponse {
  BooleanEnumTest({
    this.id,
    this.name,
    this.booleanEnum,
  ...
  final BooleanEnumTest? booleanEnum;
}

final class String$ implements ComposedSchemaBooleanEnumTest {
  const String$(this.value);

  final String value;
}

final class Integer$ implements ComposedSchemaBooleanEnumTest {
  const Integer$(this.value);

  final int value;
}

sealed class ComposedSchemaBooleanEnumTest {
  const ComposedSchemaBooleanEnumTest();

  factory ComposedSchemaBooleanEnumTest.todo(String requestBody) => requestBody;

  factory ComposedSchemaBooleanEnumTest.todo(int requestBody) => requestBody;
}
