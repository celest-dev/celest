...
class TestRegexGenerationForParameterValidationPing {
  ...
  Future<GetPingResponse> getPing() async {
    ...
    if (patternDontEscapeEscapedForwardSlash != null) {
      $request.headers['pattern_dont_escape_escaped_forward_slash'] =
          patternDontEscapeEscapedForwardSlash;
    }
    if (patternEscapeUnescapedForwardSlash != null) {
      $request.headers['pattern_escape_unescaped_forward_slash'] =
          patternEscapeUnescapedForwardSlash;
    }
    if (patternWithModifiers != null) {
      $request.headers['pattern_with_modifiers'] = patternWithModifiers;
    }
    if (patternWithBackslashAfterBracket != null) {
      $request.headers['pattern_with_backslash_after_bracket'] =
          patternWithBackslashAfterBracket;
    }
    if (alternationWithForwardSlash != null) {
      $request.headers['alternation_with_forward_slash'] =
          alternationWithForwardSlash;
    }
    if (pattenStartsEndsWithSlash != null) {
      $request.headers['patten_starts_ends_with_slash'] =
          pattenStartsEndsWithSlash;
    }
