...
import 'package:celest_cli/src/codegen/api/local_api_generator.dart';
        ...
        ..body = Block((b) {
          ...
          switch (flattenedReturnType.symbol) {
            ...
            default:
              ...
              switch (function.streamType) {
                case null:
                  b.addExpression(declareFinal('response').assign(response));
                  b.addExpression(
                    DartTypes.shelf
                        .response(
                          [literalNum(httpConfig.status)],
          ...
          final rawStatusCodes = httpConfig.statusMappings.toMap().map(
            (type, status) => MapEntry(typeHelper.fromReference(type), status),
          );
          ...
          for (final exceptionType in exceptionTypes) {
            ...
            if (statusCode == null && httpConfig.statusMappings.isNotEmpty) {
              final relevantExceptionCodes = SplayTreeMap<ast.DartType, int>((
                a,
                b,
              ) {
            ...
            if (statusCode == null &&
              ...
              final errorConfig = [
                ...exceptionElement.metadata.annotations,
                ...exceptionElement.allSupertypes.expand(
                  (it) => it.element.metadata.annotations,
                ),
          ...
          for (final exceptionType in exceptionTypes) {
            ...
            final messageExpression = switch (dartExceptionType) {
              ...
              ast.DartType(element: ast.InterfaceElement(fields: final fields))
                  when fields.any(
